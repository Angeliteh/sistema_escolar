...
from app.core.commands.alumno_commands import (
    ...
    DetallesAlumnoCommand, BuscarAlumnosPorCriterioCommand
)
...
class CommandExecutor:
    ...
    def execute_command(self, command_data: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            elif accion == "buscar_alumnos_por_criterio":
                return self._ejecutar_buscar_alumnos_por_criterio(parametros)
            ...
            elif accion == "guardar_alumno_pdf":
                return self._ejecutar_guardar_alumno_pdf(parametros)
            elif accion == "detalles_alumno":
                return self._ejecutar_detalles_alumno(parametros)
    ...
    def _ejecutar_actualizar_alumno(self, parametros: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if not alumno_id:
                return False, "Se requiere un ID o nombre de alumno para actualizar", {}
    ...
    def _ejecutar_eliminar_alumno(self, parametros: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            elif isinstance(alumno_id, str) and len(alumno_id) > 10:
            ...
            # Verificar parámetros
            if not alumno_id:
                return False, "Se requiere un ID o nombre de alumno para eliminar", {}
    ...
    def _ejecutar_generar_constancia(self, parametros: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            # Si fue exitoso, obtener los datos completos del alumno para mostrar
            if success and alumno_id:
                try:
                    # Obtener detalles completos del alumno
                    detalles_command = DetallesAlumnoCommand(alumno_id)
                    _, _, detalles_data = detalles_command.execute()

                    # Si hay detalles, añadirlos a los datos de retorno
                    if detalles_data and "alumno" in detalles_data:
                        data["alumno"] = detalles_data["alumno"]
                except Exception:
                    # Si falla, continuar con los datos originales
                    pass
    ...
    def _ejecutar_transformar_constancia(self, parametros: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            # Extraer datos del PDF para verificar si tiene calificaciones
            if tipo_destino in ["calificaciones", "traslado"]:
                try:
                    from app.core.pdf_extractor import PDFExtractor
                    extractor = PDFExtractor(ruta_archivo)
                    tiene_calificaciones = extractor.tiene_calificaciones()

                    if not tiene_calificaciones:
                        return False, f"El PDF no contiene calificaciones, no se puede generar una constancia de {tipo_destino}", {
                            "error_tipo": "calificaciones_faltantes",
                            "tipos_disponibles": ["estudio"]
                        }
                except Exception as e:
                    return False, f"Error al analizar el PDF: {str(e)}", {}

            # Crear y ejecutar el comando
            command = TransformarConstanciaCommand(ruta_archivo, tipo_destino, incluir_foto, guardar_alumno)
            success, message, data = command.execute()

            # Si fue exitoso y se guardó el alumno, obtener los datos completos del alumno para mostrar
            if success and guardar_alumno and "alumno_id" in data:
                try:
                    # Obtener detalles completos del alumno
                    detalles_command = DetallesAlumnoCommand(data["alumno_id"])
                    _, _, detalles_data = detalles_command.execute()
    ...
    def _ejecutar_guardar_alumno_pdf(self, parametros: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        """Ejecuta el comando para guardar los datos de un alumno desde un PDF sin transformarlo"""
        try:
            # Obtener parámetros
            ruta_archivo = parametros.get("ruta_archivo")
            ...
            # Verificar parámetros
            if not ruta_archivo:
                return False, "Se requiere una ruta de archivo para guardar los datos del alumno", {}
            ...
            # Extraer datos del PDF y guardar el alumno
            try:
                from app.core.pdf_extractor import PDFExtractor
                from app.services.constancia_service import ConstanciaService

                # Crear el servicio
                constancia_service = self.service_provider.constancia_service

                # Guardar alumno desde PDF
                success, message, data = constancia_service.guardar_alumno_desde_pdf(ruta_archivo)

                if success:
                    return True, "Datos del alumno guardados correctamente en la base de datos", data
                else:
                    return False, f"Error al guardar los datos del alumno: {message}", {}

            except Exception as e:
                return False, f"Error al procesar el PDF: {str(e)}", {}

        except Exception as e:
            return False, f"Error al guardar datos del alumno: {str(e)}", {}

    def _ejecutar_buscar_alumnos_por_criterio(self, parametros: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        """Ejecuta el comando para buscar alumnos por criterios específicos"""
        try:
            # Obtener parámetros
            criterio = parametros.get("criterio", "")
            valor = parametros.get("valor", "")

            # Verificar parámetros
            if not criterio:
                return False, "Se requiere un criterio para buscar alumnos", {}

            if not valor and valor != 0:  # Permitir valor 0 para grado
                return False, f"Se requiere un valor para el criterio '{criterio}'", {}

            # Validar criterios soportados
            criterios_soportados = ["grado", "grupo", "turno", "ciclo_escolar", "escuela"]
            if criterio.lower() not in criterios_soportados:
                return False, f"Criterio no soportado. Debe ser uno de: {', '.join(criterios_soportados)}", {}

            # Crear y ejecutar el comando
            command = BuscarAlumnosPorCriterioCommand(criterio, valor)
            return command.execute()

        except Exception as e:
            return False, f"Error al buscar alumnos por criterio: {str(e)}", {}

    def _ejecutar_detalles_alumno(self, parametros: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        """Ejecuta el comando para obtener detalles completos de un alumno"""
        try:
            # Obtener parámetros
            alumno_id = parametros.get("alumno_id")
            nombre = parametros.get("nombre", "")
            ...
            # Verificar parámetros
            if not alumno_id:
                return False, "Se requiere un ID o nombre de alumno para ver detalles", {}

            # Crear y ejecutar el comando
            command = DetallesAlumnoCommand(alumno_id)
            return command.execute()
