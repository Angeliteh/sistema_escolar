...
class MessageProcessor:
    ...
    def __init__(self, gemini_client=None, pdf_panel=None):
        ...
        self.action_display_config = {
            ...
            # NUEVAS ACCIONES HELPINTERPRETER - Solo respuesta humana
            "EXPLICACION_GENERAL": "never",
            "TUTORIAL_FUNCIONES": "never",
            "SOBRE_CREADOR": "never",
            "AUTO_CONSCIENCIA": "never",
            "VENTAJAS_SISTEMA": "never",
            "CASOS_AVANZADOS": "never",
            "LIMITACIONES_HONESTAS": "never",
                ...
                if conversation_stack:
                    ...
                    for i, nivel in enumerate(conversation_stack, 1):
                        ...
                        if nivel.get('data') and len(nivel['data']) <= 3:
                            ...
                            data_items = nivel['data']
                            if isinstance(data_items, list) and data_items and isinstance(data_items[0], dict) and 'nombre' in data_items[0]:
                                # Lista de alumnos
                                ...
                                # Datos estructurados
                                print(f"    │       Datos estructurados: {len(data_items)} elementos")
                ...
                if self.should_display_data(result.action, data_count):
                    ...
                    if data and data_count > 0:
                        ...
                        return True, message, result.parameters
                                     ...
                                     "AUTO_CONSCIENCIA", "VENTAJAS_SISTEMA", "CASOS_AVANZADOS",
    ...
    def add_to_conversation_stack(self, query, result_data, awaiting_type):
        ...
        if os.environ.get('DEBUG_PAUSES', 'false').lower() == 'true':
            ...
            if result_data.get('data'):
                ...
                # Verificar si data es una lista de elementos (alumnos) o datos estructurados (help)
                data_items = result_data['data']
                if isinstance(data_items, list) and data_items and isinstance(data_items[0], dict) and 'nombre' in data_items[0]:
                    # Es una lista de alumnos - mostrar como antes
                    ...
                    if len(data_items) > 3:
                        print(f"    │   ... y {len(data_items) - 3} más")
                else:
                    # Es datos estructurados (help, estadísticas, etc.) - mostrar claves principales
                    if isinstance(data_items, list) and data_items:
                        print(f"    │   Datos estructurados: {len(data_items)} elementos")
                        if isinstance(data_items[0], dict):
                            keys = list(data_items[0].keys())[:3]
                            print(f"    │   Campos principales: {', '.join(keys)}")
                    elif isinstance(data_items, dict):
                        keys = list(data_items.keys())[:3]
                        print(f"    │   Datos estructurados: {', '.join(keys)}")
                    else:
                        print(f"    │   Datos: {type(data_items).__name__}")
