...
class ChatWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 🔄 COMUNICACIÓN BIDIRECCIONAL: Estado de clarificación
        self.waiting_for_clarification = False       # Esperando clarificación del usuario
        self.clarification_data = {}                 # Datos de la solicitud de clarificación
...
Escribe "ayuda" para ver todas las funciones disponibles."""
    ...
    def send_message(self):
        ...
        if self.waiting_for_clarification and self.clarification_data:
            # 🔄 COMUNICACIÓN BIDIRECCIONAL: Estamos esperando clarificación del usuario
            self._handle_clarification_response(message_text)
            return
        elif self.waiting_for_save_confirmation and self.temp_transformed_file and self.transformation_data:
    ...
    def _handle_chat_engine_response(self, response: ChatResponse):
        ...
        # 🎨 FORMATEO AUTOMÁTICO - EVITAR DUPLICACIÓN PARA CONTEOS
        # 🔧 DETECTAR CONSULTAS DE CAMPOS ESPECÍFICOS
        is_specific_field_query = self._is_specific_field_query(response)

        should_show_text = (
            response.text and (
                # ✅ MOSTRAR texto del Master EXCEPTO para show_data (que maneja DataDisplayManager)
                response.action not in ["show_data"] or
        ...
        # 🔄 COMUNICACIÓN BIDIRECCIONAL: Manejar solicitud de aclaración
        if response.action == "solicitar_aclaracion" and response.data:
            self._handle_clarification_request(response.data)

        # Manejar datos estructurados
        elif response.action == "show_data" and response.data:
    ...
    def _is_specific_field_query(self, response: ChatResponse) -> bool:
        """
        ...
        try:
            ...
            if response.data and isinstance(response.data, dict):
                data_content = response.data.get("data", [])
                if isinstance(data_content, list) and len(data_content) <= 2:
                    ...
                    if data_content and isinstance(data_content[0], dict):
                        ...
                        if field_count <= 3:  # id + campo solicitado + tal vez uno más
                            self.logger.info(f"🔧 Consulta de campo específico detectada: {field_count} campos, {len(data_content)} registros")
                            return True

            # Verificar patrones en el texto de respuesta
            if response.text:
                text_lower = response.text.lower()
                specific_patterns = [
                    "matrícula de", "curp de", "grado de", "grupo de",
                    "fecha de nacimiento de", "turno de", "la matrícula es",
                    "el curp es", "el grado es"
                ]
                for pattern in specific_patterns:
                    if pattern in text_lower:
                        self.logger.info(f"🔧 Consulta de campo específico detectada por patrón: '{pattern}'")
                        return True
...
<p><b style="color: #88CCFF;">4.</b> Nada</p>
    ...
    def _handle_clarification_request(self, data: dict):
        """🔄 MANEJA SOLICITUD DE ACLARACIÓN DEL MASTER"""
        try:
            from datetime import datetime
            current_time = datetime.now().strftime("%H:%M:%S")

            # Extraer información de la solicitud de aclaración
            message = data.get("message", "Necesito más información")
            options = data.get("ambiguity_options", [])
            original_query = data.get("original_query", "")

            # Mostrar el mensaje de aclaración
            self.chat_list.add_assistant_message(message, current_time)

            # Si hay opciones específicas, mostrarlas de forma atractiva
            if options:
                options_text = self._format_clarification_options(options)
                self.chat_list.add_assistant_message(options_text, current_time)

            # Guardar estado de espera de clarificación
            self.waiting_for_clarification = True
            self.clarification_data = data

            self.logger.info(f"🔄 [UI] Esperando clarificación del usuario para: '{original_query}'")

        except Exception as e:
            self.logger.error(f"❌ Error manejando solicitud de aclaración: {e}")

    def _format_clarification_options(self, options: list) -> str:
        """Formatea las opciones de clarificación de manera atractiva"""
        try:
            if not options:
                return ""
            ...
            for i, option in enumerate(options, 1):
                label = option.get("label", f"Opción {i}")
                description = option.get("description", "")

                if description:
                    formatted_options.append(f"**{i}.** {label}\n   {description}")
                else:
                    formatted_options.append(f"**{i}.** {label}")
...
💡 Responde con el número de la opción que necesitas (1, 2, etc.)
"""
        except Exception as e:
            self.logger.error(f"Error formateando opciones de clarificación: {e}")
            return "Por favor, especifica qué opción necesitas."

    def _handle_clarification_response(self, user_response: str):
        """🔄 MANEJA LA RESPUESTA DEL USUARIO A UNA SOLICITUD DE CLARIFICACIÓN"""
        try:
            from datetime import datetime
            current_time = datetime.now().strftime("%H:%M:%S")

            # Extraer opciones de la solicitud de clarificación
            options = self.clarification_data.get("ambiguity_options", [])
            original_query = self.clarification_data.get("original_query", "")

            # Intentar parsear la respuesta del usuario
            selected_option = None
            user_response_clean = user_response.strip().lower()

            # Verificar si es un número
            try:
                option_number = int(user_response_clean)
                if 1 <= option_number <= len(options):
                    selected_option = options[option_number - 1]
            except ValueError:
                # No es un número, buscar por texto
                for option in options:
                    if user_response_clean in option.get("label", "").lower():
            ...
            if selected_option:
                ...
                self.chat_list.add_assistant_message(
                    f"✅ Perfecto! Procesando: {selected_option.get('label', 'opción seleccionada')}",
                    current_time
                )

                # Crear nueva consulta con la clarificación
                clarified_query = f"{original_query} - {selected_option.get('value', '')}"

                # Restablecer estado de clarificación
                self.waiting_for_clarification = False
                self.clarification_data = {}

                # Procesar la consulta clarificada
                self.logger.info(f"🔄 [UI] Procesando consulta clarificada: '{clarified_query}'")
                self._process_message_with_chat_engine(clarified_query)

            else:
                # Opción inválida
                self.chat_list.add_assistant_message(
                    f"❌ No entendí tu respuesta. Por favor, responde con el número de la opción (1, 2, etc.) o el nombre exacto.",
                    current_time
                )
                # Mantener el estado de espera de clarificación

        except Exception as e:
            self.logger.error(f"❌ Error manejando respuesta de clarificación: {e}")
