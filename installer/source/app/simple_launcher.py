...
def main():
    ...
    if missing:
        ...
        # En ejecutable, no usar input() - mostrar mensaje y continuar
        if hasattr(sys, '_MEIPASS'):  # Ejecutable
            import time
            time.sleep(3)  # Pausa de 3 segundos para que se vea el mensaje
        else:  # Desarrollo
...
if __name__ == "__main__":
    try:
        ...
        if hasattr(sys, '_MEIPASS'):  # Ejecutable
            import time
            time.sleep(5)  # Pausa de 5 segundos para que se vea el error
        else:  # Desarrollo
            input("Presiona Enter para continuar...")
        sys.exit(1)
